...
00007c09 <seta20.1>:
  ...
  # lizhi: this seta20.1 and seta20.2 is to enable a20. Before it is enabled,
  # the address bus is 20 bits and can only access 1MB memory. After it is
  # enabled, it is 32 bits and can access 4GB memory.
seta20.1:
...
seta20.2:
  ...
  #
  # Note to myself: SEG_KCODE is 1, SEG_KCODE<<3 is 8. Each entry in GDT is 8
  # bytes, and the second entry is code descriptor, so SEG_CODE<<3 is the
  # selector to the code descriptor. The ljmp will load the code descriptor to
  # cs register and $start32 to the ip register.
...
bootmain(void)
{
  ...
  // note(lizhi): actually read from the second sector. In readseg(), the
  // sector is always +1 compare to the input.
